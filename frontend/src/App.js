...
function ScrollToTop() {
  ...
  useEffect(() => {
    const handleClick = (event) => {
      ...
      if (isCalculatorLink || isDashboardFilter || href === "/") forceTop();
    };
...
function App() {
  return <div className="App"><ThemeProvider><MeasurementProvider><BrowserRouter><ScrollToTop /><Header /><Routes>
    ...
    {SPECIALIZED_CALCULATORS.map((calc) => <Route key={calc.id} path={`/${calc.slug}`} element={<SpecializedCalculatorPage />} />)}
