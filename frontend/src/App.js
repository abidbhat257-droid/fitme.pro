...
function ScrollManager() {
  ...
  useLayoutEffect(() => { if ("scrollRestoration" in window.history) window.history.scrollRestoration = "auto"; }, []);
  useEffect(() => {
    const onClick = (event) => {
      const target = event.target instanceof Element ? event.target.closest("a") : null;
      if (!target) return;
      const label = (target.textContent || "").trim().toLowerCase();
      if (label.includes("back to dashboard")) { event.preventDefault(); if (window.history.length > 1) window.history.back(); else window.location.assign("/"); }
    };
...
function App() { return <div className="App"><ThemeProvider><MeasurementProvider><BrowserRouter><ScrollManager /><Sidebar /><Header /><Routes>
  ...
  <Route path="/journal" element={<Journal />} /><Route path="/journal/editorial-standards" element={<EditorialStandards />} /><Route path="/journal/evidence-sources" element={<EvidenceSources />} /><Route path="/journal/:categorySlug" element={<JournalCategory />} /><Route path="/journal/:categorySlug/:slug" element={<JournalArticle />} />
