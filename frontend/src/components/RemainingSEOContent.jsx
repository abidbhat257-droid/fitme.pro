...
const CATEGORY_GUIDES = {
  calories: {
    ...
    reference: "Energy-balance guidance from NIH/NIDDK and general nutrition guidance from recognized public-health organizations provide useful context."
  },
  nutrition: {
    why: "Nutrition calculations turn a daily energy or nutrient target into a practical number that can help with meal planning. The result is an estimate and should be considered as part of the overall diet.",
    ...
    reference: "Dietary reference information from the NIH Office of Dietary Supplements, the National Academies, and other recognized public-health sources provides useful context."
  },
  running: {
  ...
  heart: {
    why: "Heart-rate and cardiovascular calculations provide convenient estimates for exercise intensity or basic cardiovascular measurements. They should not be treated as diagnoses.",
    ...
    reference: "Guidance from the AHA, ACSM, and other recognized cardiovascular and exercise-health organizations can provide appropriate clinical or training context."
  },
  general: {
    ...
    reference: "Interpret the result using established public-health, clinical, or exercise-science guidance appropriate to the metric."
  }
};
...
function faqs(name, formula) {
  return [
    [`What is the ${name}?`, `The ${name} uses the inputs shown above to produce a calculated estimate. The calculation method is summarized in the formula and calculation sections.`],
    ["What formula does FitMe Pro use?", formula ? `FitMe Pro uses the following method for this calculator: ${formula}.` : "FitMe Pro uses the calculation method implemented for this calculator and explains its assumptions on this page."],
    ...
    ["Why might another calculator give a different answer?", "Different calculators can use different equations, assumptions, units, rounding rules, or reference populations. Compare the methods before comparing the numbers."],
  ...
  return (
    <div className="space-y-6">
      <section>
        ...
        <p className="text-sm sm:text-base text-muted-foreground leading-relaxed">{name} is a FitMe Pro calculation designed to turn a defined set of inputs into a useful estimate. This page explains the method, inputs, interpretation, limitations, and practical uses so the result can be understood rather than viewed in isolation.</p>
      </section>
      <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Why This Calculation Matters</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">{guide.why}</p></section>
      <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Quick Answer</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">To use the {name}, enter the requested values using the units shown, calculate the result, and interpret it using the guidance below. The output is an estimate generated from the calculator's stated method.</p></section>
      <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">What Does My Result Mean?</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">Your result represents the mathematical output of the selected inputs and formula. It is most useful when compared with the relevant reference range, training target, personal baseline, or goal described for this metric—not as a standalone diagnosis or guarantee.</p></section>
      <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Formula</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed font-mono break-words">{formula}</p></section>
      <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">How It Is Calculated</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">The calculator validates the required inputs, applies the implemented equation or method, and formats the resulting value for display. When a prediction equation is used, the output is an estimate rather than a direct laboratory or clinical measurement.</p></section>
      <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Inputs Explained</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">This calculator uses: <strong>{exampleInputs}</strong>. Enter measurements carefully, keep units consistent, and avoid guessing when a reliable measurement is available.</p></section>
      <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Result Interpretation</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">Interpretation depends on the purpose of the metric. Performance values can be compared with training benchmarks, nutrition values with appropriate dietary targets, and health measurements with established clinical or public-health guidance. Context matters.</p></section>
      <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Worked Example</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">For an example, enter realistic values for {exampleInputs}, then apply <span className="font-mono">{formula}</span>. The calculator performs the arithmetic automatically. Change one input at a time to see which variables drive the output.</p></section>
      <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Factors That Affect Your Result</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">{guide.factors}</p></section>
      <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Accuracy and Limitations</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">No formula can capture every individual difference. Rounding, measurement error, changing conditions, and differences between equations can all affect the output. Use consistent inputs when tracking trends, and avoid interpreting small changes as meaningful when they may simply reflect normal measurement variation.</p></section>
      ...
      {related.length > 0 && <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Related Calculators</h3><div className="grid sm:grid-cols-2 gap-2">{related.map((item) => <Link key={item.slug} to={`/${item.slug}-calculator`} className="border border-border px-4 py-3 text-sm font-bold hover:text-[var(--brand-lime)] hover:border-[var(--brand-lime)] transition-colors">{item.name}</Link>)}</div></section>}
      <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Related FitMe Pro Guides</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">Use this calculator together with related FitMe Pro measurements and calculators to build a broader picture. Comparing complementary metrics is generally more informative than relying on a single number.</p></section>
      <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Scientific and Official References</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">{guide.reference} FitMe Pro uses original explanatory text and does not reproduce competitor articles. Reference material should be reviewed for the population, protocol, and assumptions that apply to a particular metric.</p></section>
