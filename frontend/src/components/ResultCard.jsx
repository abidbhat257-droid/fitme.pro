...
export default function ResultCard({ calc, result, ready, index = 0 }) {
  ...
  return (
    <article data-testid={CARD.root(calc.id)} className="card-in group relative bg-card border border-border p-6 brut-hover flex flex-col" style={{ animationDelay: `${Math.min(index, 20) * 30}ms`, borderTop: `3px solid ${cat.color}` }}>
      ...
      <div className="flex items-center justify-between mt-5 pt-4 border-t border-border">
        ...
        <Link to={`/${calc.slug}-calculator`} onClick={topNow} data-testid={CARD.detailsLink(calc.id)} className="text-xs font-bold uppercase tracking-[0.15em] flex items-center gap-1 hover:text-[var(--brand-lime)] transition-colors">Details <ArrowUpRight size={12} weight="bold" /></Link>
      </div>
    </article>
  );
}
