...
export default function Phase3SEOContent({ slug }) {
  ...
  return <div className="space-y-6">
    <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Quick Answer</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">{d.quick}</p></section>
    <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Formula & Method</h3><div className="border border-border bg-card p-5"><p className="font-mono text-sm leading-7">{d.formula}</p><p className="mt-3 text-sm text-muted-foreground leading-7">{d.method}</p></div></section>
    <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Inputs Explained</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">{d.inputs}</p></section>
    <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Worked Example</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">{d.example}</p></section>
    <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">What Your Result Means</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">{d.meaning}</p></section>
    <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Accuracy & Limitations</h3><p className="text-sm sm:text-base text-muted-foreground leading-relaxed">{d.limits}</p></section>
    <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Frequently Asked Questions</h3><div className="space-y-4">{d.faqs.map((q) => <div key={q} className="border border-border bg-card p-5"><h4 className="font-bold text-sm sm:text-base">{q}</h4><p className="mt-2 text-sm text-muted-foreground leading-7">This depends on the individual, the inputs used, and the calculation method. Use the result as an educational estimate and compare it with consistent measurements and real-world performance.</p></div>)}</div></section>
    <section><h3 className="font-display text-xl sm:text-2xl uppercase tracking-tight mb-3">Related Calculators</h3><div className="grid sm:grid-cols-2 gap-2">{RELATED[slug].map((path) => <Link key={path} to={path} className="border border-border px-4 py-3 text-sm font-bold hover:text-[var(--brand-lime)] hover:border-[var(--brand-lime)] transition-colors">{NAMES[path] || path}</Link>)}</div></section>
