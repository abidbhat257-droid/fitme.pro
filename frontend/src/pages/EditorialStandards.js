...
export default function EditorialStandards() {
  return <main className="mx-auto max-w-4xl px-6 py-12 sm:px-10">
    ...
    <div className="mt-10 space-y-8 text-sm leading-7">
      <section><h2 className="font-display text-2xl uppercase tracking-tight">How we research</h2><p className="mt-3">We prioritize official guidance, research institutions and primary evidence. We do not treat a single country's recommendations as universal. Important claims are cross-checked where practical, and time-sensitive guidance is reviewed before publication.</p></section>
      <section><h2 className="font-display text-2xl uppercase tracking-tight">Original writing</h2><p className="mt-3">FitMe Pro does not copy articles from WHO, CDC, NIH, NHS, NHMRC, competitors or other publishers. Sources are used to understand evidence and recommendations; our articles are independently written, structured and explained for readers.</p></section>
      ...
      <section><h2 className="font-display text-2xl uppercase tracking-tight">Our evidence framework</h2><ul className="mt-4 list-disc space-y-3 pl-6">{EVIDENCE_PRINCIPLES.map((item) => <li key={item}>{item}</li>)}</ul></section>
      <section><h2 className="font-display text-2xl uppercase tracking-tight">Core organizations</h2><div className="mt-4 space-y-4">{EVIDENCE_SOURCES.map((source) => <div key={source.id} className="rounded-xl border border-white/10 p-4"><h3 className="font-semibold">{source.name}</h3><p className="mt-1 text-muted-foreground">{source.role} · {source.topics}</p><a className="mt-2 inline-block underline" href={source.url} target="_blank" rel="noreferrer">Official source →</a></div>)}</div></section>
