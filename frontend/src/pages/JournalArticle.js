...
export default function JournalArticle() {
  ...
  const article = getJournalArticle(slug) || getExpandedJournalArticle(slug);
  useEffect(() => {
    if (!article) return;
  ...
  return <main className="min-h-screen px-4 py-10 sm:px-6 lg:px-10"><article className="mx-auto max-w-4xl">
    ...
    <div className="prose prose-invert mt-10 max-w-none">{article.sections.map(([heading,text]) => <section key={heading} className="mb-9"><h2 className="text-2xl font-bold">{heading}</h2><p className="mt-3 text-base leading-8 text-muted-foreground">{text}</p></section>)}</div>
    ...
    <section className="mt-8 border-t border-white/10 pt-6"><h2 className="text-lg font-semibold">Sources & further reading</h2><ul className="mt-3 space-y-2">{article.sources.map((s)=><li key={s.url}><a className="text-sm text-primary hover:underline" href={s.url} target="_blank" rel="noopener noreferrer">{s.label} ↗</a></li>)}</ul><p className="mt-5 text-xs leading-6 text-muted-foreground">FitMe Pro uses authoritative public-health guidance as a reference and does not reproduce source publications. Content is educational and should not replace individualized medical advice.</p></section>
  </article></main>;
}
